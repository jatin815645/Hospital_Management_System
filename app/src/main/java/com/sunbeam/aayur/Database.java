...
public class Database extends SQLiteOpenHelper {
    ...
    //username text, fullname text, address text,contactno text,pincode int,date text, amount float,otype text
    public void addOrder(String username, String fullname, String address, String contactno, int pincode, String date, String time, float amount, String otype) {
        try {
            ...
            cv.put("pincode", pincode); // Ensure pincode is an integer
            cv.put("date", date);
            cv.put("time", time); // Insert time value here
            cv.put("amount", amount);
            cv.put("otype", otype);
            db.insert("orderplace", null, cv);
            db.close();
        } catch (SQLiteException e) {
            // Handle SQLite exception (e.g., log, show error message)
            e.printStackTrace();
        }
    }
